<template>
  <el-table :data="courseWaited" highlight-current-row height="500" border style="width: 100%">
    ...
    <el-table-column prop="teachname" label="教师" width="100">
    </el-table-column>
    <el-table-column prop="credit" label="学分" width="89" sortable>
    </el-table-column>
    <el-table-column prop="coursetime" label="时间" width="100" sortable>
    </el-table-column>
    ...
    <el-table-column label="操作" fixed="right">
      <template scope="scope">
        <el-button size="small" v-if="scope.row.status=='待加入'" @click="addSchedule(scope.row)">加入</el-button>
        <el-button size="small" type="danger" @click="delCourse(scope.row)">{{scope.row.status=='待加入'?'删除':'从课表删除'}}</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
